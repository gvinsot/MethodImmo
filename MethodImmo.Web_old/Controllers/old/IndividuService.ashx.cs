...
namespace MethodImmo.Services.ViewModels
{
    ...
    /// </summary>
    public class IndividuService : GenericHandler
    {
        public override object Get(MethodImmoContext context, NameValueCollection queryString)
        {
            IndividuManager manager = new IndividuManager();
            IQueryable<Individu> query = context.PersonneSet.OfType<Individu>();
        ...
        public override ResultObject Post(string receivedJson, NameValueCollection queryString)
        {
            ...
            {
                //context.IndividuSet.Add(received);
                //context.SaveChanges();
                //id = received.Id;
            }
            return new ResultObject(HttpStatusCode.OK, "Individu créé", id.ToString());
        }
        ...
        public override ResultObject Put(string receivedJson, NameValueCollection queryString)
        {
            ...
            return new ResultObject(HttpStatusCode.OK, "Individu mis à jour","id = "+id);
        }
        ...
        public override ResultObject Delete(NameValueCollection queryString)
        {
            ...
            using (var context = new MethodImmoContext())
            {
                var toDelete = new Individu() { Id = id };
                context.PersonneSet.Attach(toDelete);
                context.PersonneSet.Remove(toDelete);
                context.SaveChanges();
            }
            return new ResultObject(HttpStatusCode.OK, "Individu supprimé", "id = " + id);
        }
