...
namespace MethodImmo.EntityJsonSerializer
{
    internal sealed class JSONSerializer
    {
        ...
        private void WriteValue(object obj)
        {
            ...
            else if (obj.GetType().Name.StartsWith("EntityReference"))// Array || obj is IList || obj is ICollection)
                WriteObject( ObjectTools.GetMemberValue(obj,"Value"));

            else if (obj.GetType().Name.StartsWith("EntityCollection"))// Array || obj is IList || obj is ICollection)
                WriteEntityCollection((IEnumerable)obj);
#endif
            else if (obj is byte[])
                WriteBytes((byte[])obj);
        ...
        //{
        //    Serialize s;
        //    JsonEntitySerializer.Instance._customSerializer.TryGetValue(obj.GetType(), out s);
        ...
        private void WriteTimeSpan(TimeSpan time)
        {
            // datetime format standard : yyyy-MM-dd HH:mm:ss
            _output.Append('\"');
            _output.Append(time.ToString("c",CultureInfo.InvariantCulture));            
            _output.Append('\"');
        }
        ...
        private void WriteObject(object obj)
        {
            ...
            if ( _current_depth <= 1 || _params.AllowedLinks != null)
            {
                List<Getters> getters = Reflection.Instance.GetGetters(t).Where(el=>el.propertyType != typeof(EntityKey) ).ToList();

                if (_params.AllowedLinks != null && _current_depth > 1)
                {
                    getters = getters.Where(
                        prop => _params.AllowedLinks.ContainsKey(obj.GetType()) &&
                                _params.AllowedLinks[obj.GetType()].Any(el => el == prop.Name)).ToList();
                }

                foreach (var prop in getters)
                {
                    object o = prop.Getter(obj);
                    ...
                    else
                    {
                        ...
                        WritePair(prop.Name, o);
                        if (o != null && _params.UseExtensions)
                        {
                            Type tt = o.GetType();
                            if (tt == typeof (System.Object))
                                map.Add(prop.Name, tt.ToString());
                        }
                        append = true;
                    }
                }
        ...
        private void WriteEntityCollection(IEnumerable array)
        {
            _output.Append('[');
